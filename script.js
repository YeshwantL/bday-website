...
async function loadSlides() {
  try {
    const res = await fetch('public/slides.json');
...
function crossFadeBackground(imageSrc, bgPosition, bgSize, blurAmount, zoomScale) {
  ...
  // Resolve path relative to public dir if needed
  const realImgSrc = imageSrc.startsWith('http') || imageSrc.startsWith('public/')
    ? imageSrc
    : 'public/' + imageSrc;

  incoming.style.transition = 'none';
  incoming.style.backgroundImage = `url('${realImgSrc}')`;
...
function preloadImage(slide) {
  ...
  const img = new Image();
  const realImgSrc = slide.image.startsWith('http') || slide.image.startsWith('public/')
    ? slide.image
    : 'public/' + slide.image;
